...
`scoring_ms_p95`, `scoring_data_prefetch_ms_p95`, `score_loop_ms_p95`
...
후보 점수 데이터를 단일 bulk JOIN 경로로 통합

검토한 대안: 최종 추천 결과 캐시와 Redis 캐시도 검토할 수 있지만 검색어·피부 프로필 조합별 폭증과 무효화 복잡도가 크다.

트레이드오프: rollup 최신성 관리와 배치 운영이 필요하다. 누락·구버전 행은 기존 온라인 계산으로 fallback해 정확성을 보존한다.

현재 단계 구현: 상품 feature read model과 사용자 preference profile을 미리 계산하고 요청에서는 유효한 버전을 조회한다.

## 7. 대응하는 transition 결과

- [이전 단계 대비 전환 수치](../../transitions/opt2-precomputed-features/README.md)
