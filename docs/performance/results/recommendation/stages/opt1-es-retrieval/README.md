...
`intent_parse_ms_p95`, `candidate_pool_ms_p95`
...
상품 특징과 사용자 선호를 rollup/read model로 사전 계산

검토한 대안: 정규식 캐시나 Aho-Corasick으로 전수 매칭 자체를 빠르게 만드는 대안도 검토했지만, 검색 책임 중복을 유지한다는 한계가 있었다.

트레이드오프: 후보 품질이 Elasticsearch 색인과 field boost 설정에 더 의존한다. 대신 Python은 점수 계산에 필요한 고민·효능 구조화에 집중한다.

현재 단계 구현: 브랜드·카테고리·상품명 탐색을 ES retrieval로 이동하고, Python hard filter 생성 경로를 축소했다.

## 7. 대응하는 transition 결과

- [이전 단계 대비 전환 수치](../../transitions/opt1-es-retrieval/README.md)
