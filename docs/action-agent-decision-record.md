...
## 2026-07-13 커머스 확장 결정

기존 조회·취소·상품 도구에 로그인 사용자의 실제 장바구니와 Toss 주문 흐름을 추가한다.

- `get_cart`: 실제 장바구니 조회
- `add_to_cart`: 상품 한 종류 즉시 추가
- `prepare_checkout`: 선택 상품과 배송지로 checkout preview 생성
- `prepare_order`: 주문 내용을 준비하고 10분간 사용자 확인 대기

개별 장바구니 추가는 즉시 실행하지만 주문 생성은 확인 API를 반드시 거친다. 확인 후 기존 주문 서비스를 호출해 `TOSS` 주문과 재고 예약을 만들며, 프론트에는 `open_payment/toss_payment`를 반환한다. 결제의 최종 승인은 사용자가 Toss 결제창에서 수행한다.

Mock 결제는 신규 주문, 프론트 실행 경로, 에이전트 tool에서 사용하지 않는다. 기존 DB 값과 비활성 API URL은 과거 데이터 및 계약 호환을 위해 남기되, Mock confirm/fail API는 항상 `410 MOCK_PAYMENT_DISABLED`를 반환한다.

상세 요청·응답 및 오류 기준은 [커머스 액션 에이전트 API 계약](api/action-agent-api-contract.md)을 따른다.

## 2026-07-14 구매 후 경험 확장 결정

구매 이후의 반복 입력을 줄이기 위해 리뷰와 반품·교환·환불 신청서 작성 지원을 추가한다.

- `prepare_review_draft`: 실제 배송완료 구매와 리뷰 작성 가능 여부를 확인하고, 사용자가 직접 말한 별점·경험만 기존 리뷰 폼에 채운다.
- `prepare_claim_draft`: 실제 배송완료 상태, 7일 신청 기간, 잔여 수량을 확인하고 사용자가 말한 신청 유형·사유만 기존 클레임 폼에 채운다.

두 tool은 화면 이동과 폼 채우기까지만 수행한다. 리뷰 공개 등록과 클레임 접수는 기존 API 계약과 화면을 유지하며 사용자가 최종 제출 버튼을 눌러야 실행된다. 따라서 에이전트가 사실이 아닌 리뷰나 하자 사유를 만들지 않고, 신청 불가능한 주문을 우회하지 않으며, 테스트를 위해 운영 데이터에 가짜 배송완료 주문을 만들지 않는다.

## 2026-07-14 배송지 등록과 checkout 연속성 결정

`prepare_checkout` 중 등록 배송지가 없으면 사용자가 배송지를 말했는데도 흐름이 중단되는 문제를 해결하기 위해 `register_shipping_address` tool을 추가한다.

- 받는 분 이름, 연락처, 우편번호, 기본 주소가 모두 확보된 경우에만 등록한다.
- 상세 주소와 배송 메모는 사용자가 제공한 경우에만 저장한다.
- 누락된 값을 LLM이 추측하지 않고 필요한 항목만 다시 질문한다.
- 사용자가 배송지 요청에 직접 답한 것을 등록 의사로 보며 별도 확인 모달은 두지 않는다.
- 첫 배송지는 기존 주소 서비스 정책에 따라 기본 배송지로 지정한다.
- 주문서 이동 중 요청된 배송지는 등록 후 checkout preview를 다시 생성하고 주문서 이동을 재개한다.
- 배송지와 연락처 원문은 agent tool 감사 로그와 브라우저의 저장된 대화 기록에 남기지 않는다.

배송지 DB 구조와 공개 주소 API는 변경하지 않고 기존 `create_user_address` 서비스를 재사용한다.
...
리뷰·클레임 폼 자동 채우기는 DB를 변경하지 않는 준비 tool이므로 별도 에이전트 확인 모달을 띄우지 않는다. 대신 기존 화면의 최종 등록 버튼을 사용자 확인 경계로 사용한다. 실제 리뷰 생성, 클레임 접수, 환불 실행을 에이전트 tool로 추가한다면 별도의 확인 정책 승인이 필요하다.
