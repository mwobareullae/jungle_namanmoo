...
상품 품질은 카테고리 prior strength 20으로 보정합니다. `review_quality_v3`는 점수 대상 외부 리뷰의 별점 80%, 재구매율 20%를 합성하고 effective sample confidence로 중립 0.5 쪽에 보정합니다. 사진 존재와 리뷰 작성일은 정보·표시·진단용으로 보존하지만 점수에는 사용하지 않습니다. 프로필 segment는 상품 전체 점수를 prior로 사용하며, effective sample size가 5보다 작은 segment도 저장합니다. 추천 단계에서는 이 작은 segment를 중립 0.5로 처리합니다.

OliveYoung seed는 전량 `MONTH_USE`이고 구매확인 원본값이 없어 두 배율을 적용하지 않습니다. 자사몰 `mubarelle` 리뷰는 원문·목록·공개 요약에는 남지만 상품 품질, 카테고리 prior, 프로필 affinity에는 반영하지 않습니다.

## v3 전환 순서

1. v3 코드를 배포합니다. DB migration은 없습니다.
2. 전체 rollup을 실행해 기존 행을 v3로 다시 계산합니다.
3. 추천 결과나 캐시를 별도로 저장하는 운영 환경이면 이전 결과를 무효화합니다.

코드만 배포하고 전체 rollup을 실행하지 않으면 기존 행의 값과 `score_version`은 이전 버전으로 남습니다. v7 추천 로더는 v3 행만 사용하므로 이 구간의 이전 리뷰 점수는 추천에서 중립 처리됩니다. 코드 배포와 재집계 완료 시점을 함께 기록합니다.
