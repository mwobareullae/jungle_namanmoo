...
REQUIRED_RAW_SOURCES = SOURCES
ALLOWED_ALIAS_TYPES = {"inci", "synonym", "en"}
EXPECTED_QUERY_VERSIONS = {
    "pubmed": "mwbl-pubmed-ingredient-skin-context-v1.1",
    "europe_pmc": "mwbl-ingredient-evidence-query-v1.1",
    "crossref": "mwbl-ingredient-evidence-query-v1.1",
    "openalex": "mwbl-ingredient-evidence-query-v1.1",
    "kci": "mwbl-kci-approved-term-skin-context-v1.1",
    "riss": "mwbl-riss-approved-term-skin-v1.1",
}
EXPECTED_PIPELINE_VERSIONS = {
    "pubmed": "mwbl-ingredient-evidence-filter-v1.1-fresh-pubmed",
    "europe_pmc": "mwbl-ingredient-multisource-discovery-v1.1-fresh",
    "crossref": "mwbl-ingredient-multisource-discovery-v1.1-fresh",
    "openalex": "mwbl-ingredient-multisource-discovery-v1.1-fresh",
    "kci": "mwbl-ingredient-evidence-filter-v1.1-fresh-kci",
    "riss": "mwbl-ingredient-evidence-filter-v1.1-fresh-riss",
}
...
SOURCE_CANDIDATE_ID_FIELD = {
    "pubmed": "pmid",
    "europe_pmc": "source_id",
    "crossref": "source_id",
    "openalex": "source_id",
    "riss": "control_no",
}
RISS_LINKED_CANDIDATE_FIELDS = {
    "query_id",
    "query_sha256",
    "canonical_name_en",
    "source_id",
        ...
        if pagination == "complete":
            ...
            if source == "riss":
                if total is None or returned is None or total < returned:
                    errors.append("RISS complete rollup has impossible deduplicated count")
            elif total is None or returned is None or total != returned:
            ...
            if source == "riss":
                if cap is None or returned is None or returned > cap:
                    errors.append("RISS rollup candidates exceed aggregate retrieval cap")
            elif cap is None or returned is None or returned != cap:
...
def _source_auxiliary_paths(fresh_dir: Path, source: str) -> dict[str, Path]:
    if source != "riss":
        return {}
    return {
        "execution_log": fresh_dir / "riss_execution_log_466_v1_1_fresh.csv",
        "execution_hits": fresh_dir / "riss_execution_hits_466_v1_1_fresh.csv",
    }
...
def _summary_count(
    ...
    *,
    ...
    return _first_integer(
        summary,
        "candidate_row_count",
        "deduplicated_candidate_row_count",
...
def _validate_riss_auxiliary(
    *,
    ...
    hit_rows: Sequence[Mapping[str, str]],
) -> tuple[
    list[dict[str, object]],
    dict[str, Mapping[str, str]],
    dict[tuple[str, str], list[Mapping[str, str]]],
]:
    """Validate the per-approved-term RISS audit trail and hit linkage."""

    violations: list[dict[str, object]] = []
    expected_plan: list[dict[str, str]] = []
    for manifest in manifest_rows:
        ingredient_id = manifest.get("ingredient_id", "")
        provenance = list(approved_provenance[ingredient_id])
        for index, term in enumerate(approved_terms[ingredient_id]):
            search_query = f'"{_clean_quoted_term(term)}" skin'
            query_sha = text_sha256(search_query)
            term_provenance = provenance[index]
            expected_plan.append(
                {
                    "execution_query_id": f"riss:{ingredient_id}:{query_sha[:16]}",
                    "query_sha256": query_sha,
                    "ingredient_id": ingredient_id,
                    "canonical_name_en": approved_terms[ingredient_id][0],
                    "approved_search_term": term,
                    "term_origin": term_provenance.get("origin", ""),
                    "alias_type": term_provenance.get("alias_type", ""),
                    "term_source": term_provenance.get("source", ""),
                    "search_query": search_query,
                }
            )

    expected_ids = [row["execution_query_id"] for row in expected_plan]
    actual_ids = [row.get("execution_query_id", "") for row in execution_rows]
    if actual_ids != expected_ids:
        violations.append(
            {
                "reason": "execution_plan_not_exact_approved_term_order",
                "actual_rows": len(actual_ids),
                "expected_rows": len(expected_ids),
                "first_actual": actual_ids[:3],
                "first_expected": expected_ids[:3],
            }
        )

    execution_by_id: dict[str, Mapping[str, str]] = {}
    expected_by_id = {row["execution_query_id"]: row for row in expected_plan}
    for row in execution_rows:
        execution_id = row.get("execution_query_id", "")
        ingredient_id = row.get("ingredient_id", "")
        if not execution_id or execution_id in execution_by_id:
            violations.append(
                {"ingredient_id": ingredient_id, "reason": "duplicate_or_blank_execution_id"}
            )
        execution_by_id[execution_id] = row
        expected = expected_by_id.get(execution_id)
        if expected is None:
            violations.append(
                {"ingredient_id": ingredient_id, "reason": "unexpected_execution_id"}
            )
            continue
        for field, expected_value in expected.items():
            if row.get(field, "") != expected_value:
                violations.append(
                    {
                        "ingredient_id": ingredient_id,
                        "reason": f"execution_{field}_mismatch",
                    }
                )
        fixed_values = {
            "source": "riss",
            "query_version": EXPECTED_QUERY_VERSIONS["riss"],
            "query_contract_status": "approved_terms_only",
            "review_status": "candidate_unverified",
            "runtime_score_change": "none",
            "pipeline_version": EXPECTED_PIPELINE_VERSIONS["riss"],
        }
        ...
        status = row.get("request_status", "").casefold()
        pagination = row.get("pagination_status", "").casefold()
        complete = row.get("pagination_complete", "").upper()
        rerun = row.get("rerun_required", "").upper()
        raw_count = _integer(row.get("raw_hit_count"))
        returned_count = _integer(row.get("returned_count"))
        retrieval_cap = _integer(row.get("retrieval_cap"))
        if status == "success":
            if rerun != "N":
                violations.append(
                    {"ingredient_id": ingredient_id, "reason": "execution_success_rerun"}
                )
            if pagination == "complete":
                if complete != "Y" or raw_count is None or raw_count != returned_count:
                    violations.append(
                        {"ingredient_id": ingredient_id, "reason": "execution_complete_counts"}
                    )
            elif pagination == "capped_at_page_scale":
                if (
                    complete != "N"
                    or raw_count is None
                    or returned_count is None
                    or retrieval_cap is None
                    or raw_count <= returned_count
                    or returned_count != retrieval_cap
                ):
                    violations.append(
                        {"ingredient_id": ingredient_id, "reason": "execution_capped_counts"}
                    )
            else:
                violations.append(
                    {"ingredient_id": ingredient_id, "reason": "execution_pagination_status"}
                )
        elif rerun != "Y" or pagination != "failed" or complete != "N":
            violations.append(
                {"ingredient_id": ingredient_id, "reason": "execution_failure_truth_table"}
            )

    hits_by_execution: dict[str, list[Mapping[str, str]]] = defaultdict(list)
    hits_by_paper: dict[tuple[str, str], list[Mapping[str, str]]] = defaultdict(list)
    ...
    for row in hit_rows:
        ...
        if execution is None:
            violations.append(
                {"ingredient_id": ingredient_id, "reason": "hit_unknown_execution_id"}
            )
            continue
        for field in ("query_sha256", "ingredient_id", "approved_search_term"):
            if row.get(field, "") != execution.get(field, ""):
                violations.append(
                    {"ingredient_id": ingredient_id, "reason": f"hit_{field}_mismatch"}
                )
        fixed_values = {
            "review_status": "candidate_unverified",
            "runtime_score_change": "none",
        ...
        if position is None or position <= 0:
            violations.append(
                {"ingredient_id": ingredient_id, "reason": "hit_invalid_result_position"}
            )
        else:
            positions[execution_id].append(position)
        control_no = row.get("control_no", "")
        if not control_no:
            violations.append(
                {"ingredient_id": ingredient_id, "reason": "hit_control_no_blank"}
            )
        hits_by_execution[execution_id].append(row)
        hits_by_paper[(ingredient_id, control_no)].append(row)

    for execution_id, execution in execution_by_id.items():
        returned_count = _integer(execution.get("returned_count"))
        linked_count = len(hits_by_execution.get(execution_id, []))
        if returned_count != linked_count:
            violations.append(
                {
                    "ingredient_id": execution.get("ingredient_id", ""),
                    "reason": "execution_hit_count_mismatch",
                    "returned_count": returned_count,
                    "hit_count": linked_count,
                }
            )
        actual_positions = sorted(positions.get(execution_id, []))
        if actual_positions != list(range(1, linked_count + 1)):
            violations.append(
                {
                    "ingredient_id": execution.get("ingredient_id", ""),
                    "reason": "execution_hit_positions_not_contiguous",
                }
            )
    return violations, execution_by_id, hits_by_paper
...
def validate_source_package(
    *,
    ...
    auxiliary_paths = _source_auxiliary_paths(fresh_dir, source)
    ...
    auxiliary_data: dict[str, tuple[list[str], list[dict[str, str]]]] = {
        kind: read_csv_with_fields(path)
        for kind, path in auxiliary_paths.items()
    }
    raw_sha256.update({str(path): sha256(path) for path in all_paths.values()})
    try:
    ...
    if source in STANDARD_LINKED_CANDIDATE_SOURCES:
        ...
        missing_candidate_fields.extend(
            sorted({SOURCE_CANDIDATE_ID_FIELD[source]} - set(candidate_fields))
        )
        if source == "pubmed":
            missing_candidate_fields.extend(sorted({"title"} - set(candidate_fields)))
    elif source == "riss":
        missing_candidate_fields.extend(
            sorted((RISS_LINKED_CANDIDATE_FIELDS | {"source_url"}) - set(candidate_fields))
        )
    assert_true(
        checks,
    ...
    for kind, (fields, _) in auxiliary_data.items():
        missing = sorted(RISS_AUXILIARY_FIELDS[kind] - set(fields))
        assert_true(
            checks,
            f"{source}_{kind}_schema",
            not missing,
    ...
    for row in query_rows:
        ...
        for reason in _log_semantic_errors(row, source=source):
        ...
        if row.get("query_version", "") != EXPECTED_QUERY_VERSIONS[source]:
            state_violations.append(
                {"ingredient_id": ingredient_id, "reason": "query_version_mismatch"}
            )
        if row.get("pipeline_version", "") != EXPECTED_PIPELINE_VERSIONS[source]:
            state_violations.append(
                {"ingredient_id": ingredient_id, "reason": "pipeline_version_mismatch"}
            )
    ...
    execution_by_id: dict[str, Mapping[str, str]] = {}
    riss_hits_by_paper: dict[
        tuple[str, str], list[Mapping[str, str]]
    ] = defaultdict(list)
    if source == "riss":
        _, execution_rows = auxiliary_data["execution_log"]
        _, execution_hit_rows = auxiliary_data["execution_hits"]
        auxiliary_violations, execution_by_id, riss_hits_by_paper = (
            _validate_riss_auxiliary(
                manifest_rows=manifest_rows,
                approved_terms=approved_terms,
                approved_provenance=approved_provenance,
                execution_rows=execution_rows,
                hit_rows=execution_hit_rows,
            )
        )
        executions_by_ingredient: dict[
            str, list[Mapping[str, str]]
        ] = defaultdict(list)
        for execution in execution_rows:
            executions_by_ingredient[execution.get("ingredient_id", "")].append(
                execution
            )
        for rollup in query_rows:
            ingredient_id = rollup.get("ingredient_id", "")
            executions = executions_by_ingredient.get(ingredient_id, [])
            failed = [
                row for row in executions if row.get("request_status", "") != "success"
            ]
            capped = [
                row
                for row in executions
                if row.get("pagination_status", "") == "capped_at_page_scale"
            ]
            expected_values = {
                "execution_count": len(executions),
                "successful_execution_count": len(executions) - len(failed),
                "failed_execution_count": len(failed),
                "capped_execution_count": len(capped),
                "total_hit_count": sum(
                    _integer(row.get("raw_hit_count")) or 0 for row in executions
                ),
                "raw_hit_count": sum(
                    _integer(row.get("raw_hit_count")) or 0 for row in executions
                ),
                "retrieval_cap": sum(
                    _integer(row.get("retrieval_cap")) or 0 for row in executions
                ),
            }
            for field, expected_value in expected_values.items():
                if _integer(rollup.get(field)) != expected_value:
                    auxiliary_violations.append(
                        {
                            "ingredient_id": ingredient_id,
                            "reason": f"rollup_{field}_mismatch",
                            "actual": rollup.get(field, ""),
                            "expected": expected_value,
                        }
                    )
            expected_status = "error" if failed else "success"
            expected_pagination = (
                "failed"
                if failed
                else "capped_at_page_scale"
                if capped
                else "complete"
            )
            expected_complete = "Y" if expected_pagination == "complete" else "N"
            expected_rerun = "Y" if failed else "N"
            for field, expected_value in {
                "request_status": expected_status,
                "pagination_status": expected_pagination,
                "pagination_complete": expected_complete,
                "rerun_required": expected_rerun,
            }.items():
        ...
        assert_true(
            checks,
            "riss_execution_plan_hits_and_rollups",
            not auxiliary_violations,
            auxiliary_violations[:30],
        )

    candidate_violations: list[dict[str, object]] = []
    candidate_counts: Counter[str] = Counter()
    candidate_positions: dict[str, list[int]] = defaultdict(list)
    candidate_identity_seen: set[tuple[str, str]] = set()
    for row in candidate_rows:
        ...
        identity_field = SOURCE_CANDIDATE_ID_FIELD.get(source)
        if identity_field:
            identity = row.get(identity_field, "").strip()
            identity_key = (ingredient_id, identity)
            if not identity:
                candidate_violations.append(
                    {
                        "ingredient_id": ingredient_id,
                        "reason": f"blank_{identity_field}",
                    }
                )
            elif identity_key in candidate_identity_seen:
                candidate_violations.append(
                    {
                        "ingredient_id": ingredient_id,
                        "reason": f"duplicate_{identity_field}_within_ingredient",
                    }
                )
            candidate_identity_seen.add(identity_key)
        if source == "pubmed" and (
            not row.get("pmid", "").isdigit() or not row.get("title", "").strip()
        ):
            candidate_violations.append(
                {"ingredient_id": ingredient_id, "reason": "invalid_pubmed_metadata"}
            )
        if source in STANDARD_LINKED_CANDIDATE_SOURCES and (
            not row.get("metadata_status", "").strip()
            or not row.get("source_url", "").strip()
        ):
            candidate_violations.append(
                {"ingredient_id": ingredient_id, "reason": "candidate_metadata_audit_gap"}
            )
        if source in STANDARD_LINKED_CANDIDATE_SOURCES:
            ...
            if query is None:
                candidate_violations.append(
                    {"ingredient_id": ingredient_id, "reason": "unknown_rollup_query_id"}
                )
            else:
                for field in ("ingredient_id", "canonical_name_en", "query_sha256"):
                    if row.get(field, "") != query.get(field, ""):
                        candidate_violations.append(
                            {
                                "ingredient_id": ingredient_id,
                                "reason": f"candidate_rollup_{field}_mismatch",
                            }
                        )
            execution_ids = [
                value
            ...
            linked_executions = [execution_by_id.get(value) for value in execution_ids]
            if (
                not execution_ids
                or any(value is None for value in linked_executions)
                or any(
                    value is not None and value.get("ingredient_id", "") != ingredient_id
                    for value in linked_executions
                )
            ):
                candidate_violations.append(
                    {
            ...
            control_no = row.get("control_no", "")
            if not control_no or row.get("source_id", "") != control_no:
                candidate_violations.append(
                    {"ingredient_id": ingredient_id, "reason": "candidate_control_no_mismatch"}
                )
            if not row.get("source_url", "").strip():
                candidate_violations.append(
                    {"ingredient_id": ingredient_id, "reason": "candidate_source_url_blank"}
                )
            linked_hits = riss_hits_by_paper.get((ingredient_id, control_no), [])
            expected_execution_ids = _stable_unique(
                [hit.get("execution_query_id", "") for hit in linked_hits]
            )
            expected_terms = _stable_unique(
                [hit.get("approved_search_term", "") for hit in linked_hits]
            )
            if not linked_hits or execution_ids != expected_execution_ids:
                candidate_violations.append(
                    {
                        "ingredient_id": ingredient_id,
                        "reason": "candidate_not_exactly_linked_to_execution_hits",
                    }
                )
            if row.get("matched_search_terms", "").split("|") != expected_terms:
                candidate_violations.append(
                    {
                        "ingredient_id": ingredient_id,
                        "reason": "candidate_matched_search_terms_mismatch",
                    }
                )
        elif candidate_rows:
            candidate_violations.append(
                {
                    "ingredient_id": ingredient_id,
                    "reason": "access_unavailable_source_has_candidates",
                }
            )
    ...
    for row in query_rows:
        ...
        elif candidate_counts[ingredient_id] != returned:
            candidate_violations.append(
                {
                    "ingredient_id": ingredient_id,
    ...
    if summary.get("pipeline_version") != EXPECTED_PIPELINE_VERSIONS[source]:
        summary_violations.append({"reason": "summary_pipeline_version_mismatch"})
    if (
        summary.get("query_version") is not None
        and summary.get("query_version") != EXPECTED_QUERY_VERSIONS[source]
    ):
        summary_violations.append({"reason": "summary_query_version_mismatch"})
    summary_file_rows: dict[str, tuple[Path, int, int | None]] = {
        "query_log": (
            paths["query_log"],
            len(query_rows),
            _summary_count(summary, kind="query_log"),
        ),
        "candidates": (
            paths["candidates"],
            len(candidate_rows),
            _summary_count(summary, kind="candidates"),
        ),
    }
    if source == "riss":
        summary_file_rows.update(
            {
                "execution_log": (
                    auxiliary_paths["execution_log"],
                    len(auxiliary_data["execution_log"][1]),
                    _first_integer(summary, "approved_term_execution_count"),
                ),
                "execution_hits": (
                    auxiliary_paths["execution_hits"],
                    len(auxiliary_data["execution_hits"][1]),
                    _first_integer(summary, "execution_hit_row_count"),
                ),
            }
        )
    for kind, (path, actual_count, declared_count) in summary_file_rows.items():
    ...
    declared_request_statuses = summary.get("request_status_counts")
    if isinstance(declared_request_statuses, Mapping):
        actual_request_statuses = dict(
            Counter(row.get("request_status", "") for row in query_rows)
        )
        if dict(declared_request_statuses) != actual_request_statuses:
            summary_violations.append(
                {"reason": "summary_request_status_counts_mismatch"}
            )
    assert_true(
        checks,
    ...
    return (
        {
            ...
            "auxiliary_rows": {
                kind: len(rows) for kind, (_, rows) in auxiliary_data.items()
            },
...
def validate(args: argparse.Namespace) -> dict[str, object]:
    ...
    for source in SOURCES:
        status, rows = validate_source_package(
            ...
            optional=source not in REQUIRED_RAW_SOURCES,
        )
