...
import { createProductReview, getReviewableOrderItems } from "../../lib/reviewApi";
import type { ReviewableOrderItem } from "../../types/review";
...
function ReviewWritePage() {
  ...
  useEffect(() => {
    ...
    getReviewableOrderItems()
      .then((response) => {
        if (isMounted) setItems(response.items.filter((item) => item.can_write));
      })
      .catch(() => {
        if (isMounted) {
          setItems([]);
          setErrorMessage("작성 가능한 리뷰 목록을 불러오지 못했습니다.");
        }
      })
      .finally(() => {
  ...
  return (
    <MyPageLayout>
      ...
      <div className="review-write-page">
        <section className="review-write-page__items" aria-label="리뷰 작성 가능 상품">
          {isLoading ? (
