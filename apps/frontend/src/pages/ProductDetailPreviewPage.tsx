    ...
    <>
      ...
      <section className="naver-preview-product-top">
        ...
        <div className="naver-preview-info">
          ...
          <div className="naver-preview-buy-grid"><button className="buy" disabled={isPurchasePending || isSoldOut} type="button" onClick={() => void handlePurchase()}>{isSoldOut ? "일시품절" : isPurchasePending ? "주문서 준비 중" : "구매하기"}</button><button className={product && wishedProductIds.has(product.product_id) ? "is-wished" : ""} type="button" onClick={() => product && void toggleWishlist(product.product_id)}><HeartIcon size={20} />찜하기</button><button data-agent-cart-target disabled={isSoldOut} type="button" onClick={() => void handleAddToCart()}><ShoppingBagIcon size={20} />{isSoldOut ? "일시품절" : "장바구니"}</button></div>
        </div>
      </section>
