...
function ProductDetailPreviewPage() {
  ...
  useEffect(() => {
    ...
    if (!productId) return;
    let isMounted = true;
    api.getProduct(productId).then(async (response) => {
      if (!isMounted) return;
      setProduct(response);
      try {
