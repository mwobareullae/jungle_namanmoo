...
export default function ScoreAnalysisPanel({ product }: Props) {
  ...
  return <article className="naver-preview-score-analysis"><h2>점수분석</h2><div className="naver-preview-score-intro"><strong>최종 추천 점수 {Math.round(product.total_score)}점</strong><span>{product.reason_summary || "상품 정보를 여러 기준으로 살펴본 결과예요."}</span></div><div className="naver-preview-score-controls"><button type="button" onClick={() => setOpenAxes(new Set(axes.map((axis) => axis.key)))}>전체 펼치기</button><button type="button" onClick={() => setOpenAxes(new Set())}>전체 닫기</button></div><div className="naver-preview-score-grid">{axes.map((axis) => {
    const weight = weightFor(weights, axis.key);
    const contribution = typeof axis.score === "number" && typeof weight === "number" ? axis.score * weight / 100 : undefined;
    const isUnavailable = axis.status === "데이터 부족" || axis.status === "정보 없음";
    const hasContribution = !isUnavailable && typeof contribution === "number" && typeof weight === "number";

    return <details className="naver-preview-score-card" key={axis.key} open={openAxes.has(axis.key)} onToggle={(event) => { const nextOpen = event.currentTarget.open; setOpenAxes((current) => { const next = new Set(current); if (nextOpen) next.add(axis.key); else next.delete(axis.key); return next; }); }}><summary><span>{axis.label}{axis.status === "데이터 부족" ? <em className="naver-preview-score-status is-neutral">데이터 부족</em> : null}</span><strong className={`naver-preview-score-contribution${hasContribution ? "" : " is-unavailable"}`}>{hasContribution ? <><b>{contribution.toFixed(1)}점</b><small>/ {weight.toFixed(1)}점</small></> : "점수 미반영"}</strong></summary><p>{axis.description}</p><div className="naver-preview-score-metrics"><span>기준 적합도 {typeof axis.score === "number" ? `${Math.round(axis.score)}점 / 100점` : "-"}</span><span>반영 비중 {typeof weight === "number" ? `${weight.toFixed(1)}%` : "-"}</span><span>최종 기여 {hasContribution ? `${contribution.toFixed(1)}점 / ${weight.toFixed(1)}점` : "점수 미반영"}</span></div></details>;
  })}</div><div className={`naver-preview-score-warning${riskCount || riskPenalty > 0 ? " is-risk" : ""}`}>{riskCount || riskPenalty > 0 ? <>주의가 필요한 성분 {riskCount}개 · 추천 점수에 {riskPenalty}점 반영{breakdown.risk_warnings?.length ? <ul>{breakdown.risk_warnings.map((warning) => <li key={warning}>{warning}</li>)}</ul> : null}</> : "주의 없음 · 확인된 위험 성분이 없어요."}</div></article>;
}
