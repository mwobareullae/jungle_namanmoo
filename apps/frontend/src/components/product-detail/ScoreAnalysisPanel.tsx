...
const labels: Array<{ key: string; label: string; group: string }> = [
  ...
  { key: "skin_profile_score", label: "피부 타입 적합도", group: "내 피부와의 적합도" }, { key: "skin_test_score", label: "피부 고민 테스트 반영", group: "내 피부와의 적합도" }, { key: "behavior_personalization_score", label: "관심·사용 기록 반영", group: "내 피부와의 적합도" }, { key: "review_profile_affinity_score", label: "비슷한 피부 리뷰", group: "내 피부와의 적합도" },
  { key: "search_match_score", label: "입력한 고민과의 일치", group: "구매 조건과 인기" }, { key: "price_value_score", label: "가격 조건", group: "구매 조건과 인기" }, { key: "market_popularity_score", label: "상품 인기", group: "구매 조건과 인기" }, { key: "review_quality_score", label: "리뷰 만족도 데이터", group: "구매 조건과 인기" },
];

function weightFor(weights: Record<string, number> | undefined, key: string) {
  const aliases = [key, key.replace(/_score$/, ""), key === "skin_profile_score" ? "skin_type_match_score" : "", key === "search_match_score" ? "keyword_score" : ""].filter(Boolean);
...
function axisData(product: ProductDetail): Axis[] {
  ...
  return labels.map(({ key, label }) => {
    ...
    if (numeric === undefined || (key === "functional_claim_score" || key === "skin_test_score" || key === "behavior_personalization_score" || key === "market_popularity_score")) return { key, label, status: "정보 없음", description: "이 기준에 사용할 정보가 없어 추천 점수에 반영하지 않았어요." };
