...
SUGGESTION_QUERIES="${SUGGESTION_QUERIES:-}"
SEARCH_FEATURE_FILTER="${SEARCH_FEATURE_FILTER:-moisturizing_calming}"
SEARCH_SKIN_TYPE_FILTER="${SEARCH_SKIN_TYPE_FILTER:-dehydrated_oily}"
DEBUG_ERRORS="${DEBUG_ERRORS:-false}"
SLA_MS="${SLA_MS:-3000}"
CATALOG_SEARCH_SLA_MS="${CATALOG_SEARCH_SLA_MS:-500}"
CATALOG_SUGGESTIONS_SLA_MS="${CATALOG_SUGGESTIONS_SLA_MS:-200}"
...
run_k6() {
  ...
  (
    ...
    k6 run \
      ...
      -e SUGGESTION_QUERIES="$SUGGESTION_QUERIES" \
      -e SEARCH_FEATURE_FILTER="$SEARCH_FEATURE_FILTER" \
      -e SEARCH_SKIN_TYPE_FILTER="$SEARCH_SKIN_TYPE_FILTER" \
      -e DEBUG_ERRORS="$DEBUG_ERRORS" \
      -e SLA_MS="$SLA_MS" \
      -e CATALOG_SEARCH_SLA_MS="$CATALOG_SEARCH_SLA_MS" \
      -e CATALOG_SUGGESTIONS_SLA_MS="$CATALOG_SUGGESTIONS_SLA_MS" \
...
generate_summary() {
  ...
  python3 "$SCRIPT_DIR/generate_report.py" \
    ...
    --sla-ms "$SLA_MS" \
    --catalog-search-sla-ms "$CATALOG_SEARCH_SLA_MS" \
    --catalog-suggestions-sla-ms "$CATALOG_SUGGESTIONS_SLA_MS" \
    --out "$REPORT_MD"
}
